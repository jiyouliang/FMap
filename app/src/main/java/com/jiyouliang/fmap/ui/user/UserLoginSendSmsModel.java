...
public class UserLoginSendSmsModel extends BaseModel implements IUserLoginSendSmsModel{
    @Override
    public void loginBySms(Context context, String mobile, String sms, HttpTaskClient.OnHttpResponseListener listener) throws Exception {
        HashMap<String, String> params = new HashMap<>();
        params.put("mobile", mobile);
        params.put("sms", sms);
        String json = assembleReqParams(context, params);
        HttpTaskClient.getInstance().loginBySms(json, UserLoginData.class, listener);
    }
